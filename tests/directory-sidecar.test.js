...
const { matchesLegacyDirectoryMarker } = await import(
	pathToFileURL(path.resolve('scripts/migrate-directory-sidecars.mjs')).href,
);
...
test('migration script treats missing legacy lock expiresAt as matching an existing sidecar', () => {
	const metadata = {
		resourcetype: '<collection />',
	...
	assert.equal(matchesLegacyDirectoryMarker(existingSidecar, metadata), true);
});

test('migration script ignores legacy lock order when matching an existing sidecar', () => {
	const metadata = {
		resourcetype: '<collection />',
		lock_records: JSON.stringify([
			{
...
function createObject(key, options = {}) {
	...
	return {
		...
		size: options.size ?? 1,
		etag,
		httpEtag: options.httpEtag ?? `"${etag}"`,
		uploaded: options.uploaded ?? new Date('2024-01-01T00:00:00.000Z'),
...
function createListingBucket(objects) {
	...
	return {
		...
		async get(key, options = {}) {
			...
			const fullBody = new TextEncoder().encode(bodyText);
			let body = fullBody;
			let range;
			let rangeHeader = options.range?.get?.('Range');
			let suffix = Number(rangeHeader?.match(/^bytes=-(\d+)$/)?.[1] ?? NaN);
			if (Number.isFinite(suffix)) {
				let start = Math.max(fullBody.length - suffix, 0);
				body = fullBody.slice(start);
				range = { suffix };
			}
			return {
				body,
	...
	return {
		...
		async get(key) {
			...
			return {
				...
				httpEtag: object.httpEtag ?? `"${object.etag ?? 'etag'}"`,
...
test('PUT returns ETag and Last-Modified headers for stored objects', async () => {
	const bucket = createTrackingBucket();
	...
	assert.equal(response.headers.get('Last-Modified'), 'Mon, 01 Jan 2024 00:00:00 GMT');
});

test('PUT rejects stale If-Unmodified-Since for an existing object', async () => {
	const bucket = createTrackingBucket([
		createObject('file.txt', {
			bodyText: 'old',
	...
	assert.equal(bucket.operations.put.length, 0);
});

test('PUT rejects collection paths with query strings', async () => {
	const bucket = createTrackingBucket();
	const response = await handlePut(
		new Request('http://example.com/docs/?download=1', {
			method: 'PUT',
			body: 'data',
		}),
		bucket,
	...
	assert.equal(response.headers.get('Allow')?.includes('PUT'), true);
...
test('GET preserves directory listing behavior when the URL has a query string', async () => {
	const bucket = createListingBucket([createObject('docs/readme.txt')]);
	const response = await handleGet(new Request('http://example.com/docs/?v=1', { method: 'GET' }), bucket);

	assert.equal(response.status, 200);
	assert.match(await response.text(), /readme\.txt/);
});

test('GET directory listing uses the resource key instead of Content-Disposition', async () => {
	const bucket = createListingBucket([
		createObject('docs/readme.txt', {
			httpMetadata: { contentDisposition: 'attachment; filename="ignored.txt"' },
		}),
	]);
	const response = await handleGet(new Request('http://example.com/docs/', { method: 'GET' }), bucket);

	assert.equal(response.status, 200);
	const body = await response.text();
	assert.match(body, /readme\.txt/);
	assert.doesNotMatch(body, /ignored\.txt/);
});

test('GET returns ETag and Last-Modified headers for file objects', async () => {
	const bucket = createListingBucket([createObject('docs/readme.txt')]);
	const request = new Request('http://example.com/docs/readme.txt', { method: 'GET' });
...
test('GET handles suffix ranges without producing a negative content offset', async () => {
	const bucket = createListingBucket([createObject('docs/readme.txt', { bodyText: 'hello', size: 5 })]);
	const response = await handleGet(
		new Request('http://example.com/docs/readme.txt', {
			method: 'GET',
...
test('PROPFIND returns quoted getetag values that match file response headers', async () => {
	const bucket = createListingBucket([createObject('file.txt')]);
	const request = new Request('http://example.com/file.txt', {
		method: 'PROPFIND',
		headers: { Depth: '0' },
	});
	...
	assert.equal(response.status, 207);
	const body = await response.text();
	assert.match(body, /<getetag>&quot;etag&quot;<\/getetag>/);
});

test('PROPFIND returns ISO 8601 creationdate for file resources', async () => {
	const bucket = createListingBucket([
		createObject('file.txt', {
			uploaded: new Date('2024-01-01T00:00:00.000Z'),
		}),
	]);
	const response = await handlePropfind(
		new Request('http://example.com/file.txt', {
			method: 'PROPFIND',
			headers: { Depth: '0' },
		}),
		bucket,
	...
	assert.equal(response.status, 207);
	assert.match(await response.text(), /<creationdate>2024-01-01T00:00:00\.000Z<\/creationdate>/);
});

test('PROPFIND does not invent timestamps for implicit directories', async () => {
	const bucket = createListingBucket([createObject('docs/readme.txt')]);
	const response = await handlePropfind(
	...
	assert.equal(response.status, 207);
	const body = await response.text();
	assert.doesNotMatch(body, /<creationdate>/);
	assert.doesNotMatch(body, /<getlastmodified>/);
});

test('PROPFIND uses stable sidecar timestamps for sidecar-only directories', async () => {
	const bucket = createListingBucket([
		createObject('.__sidecar__/empty.json', {
			uploaded: new Date('2024-01-03T00:00:00.000Z'),
		}),
	]);
	const response = await handlePropfind(
		new Request('http://example.com/empty/', {
			method: 'PROPFIND',
			headers: { Depth: '0' },
		}),
		bucket,
	...
	assert.equal(response.status, 207);
	const body = await response.text();
	assert.match(body, /<creationdate>2024-01-03T00:00:00\.000Z<\/creationdate>/);
	assert.match(body, /<getlastmodified>Wed, 03 Jan 2024 00:00:00 GMT<\/getlastmodified>/);
});

test('PROPFIND returns displayname based on the resource key instead of Content-Disposition', async () => {
	const bucket = createListingBucket([
		createObject('docs/readme.txt', {
			httpMetadata: { contentDisposition: 'attachment; filename="ignored.txt"' },
		}),
	...
	assert.equal(response.status, 207);
	const body = await response.text();
	assert.match(body, /<displayname>readme\.txt<\/displayname>/);
	assert.doesNotMatch(body, /ignored\.txt/);
});
...
test('PROPFIND ignores malformed dead property metadata on file resources', async () => {
	const bucket = createListingBucket([
		createObject('file.txt', {
			bodyText: 'hello',
			customMetadata: {
	...
	assert.equal(response.status, 207);
	const body = await response.text();
	assert.match(body, /<displayname>file\.txt<\/displayname>/);
	assert.equal(body.includes('Legacy'), false);
});
...
test('PROPPATCH rejects stale If-Unmodified-Since for file resources', async () => {
	const bucket = createTrackingBucket([
		createObject('file.txt', {
			bodyText: 'hello',
			uploaded: new Date('2024-01-02T00:00:00.000Z'),
...
test('MKCOL rejects implicit ancestor created by nested sidecar', async () => {
	...
	assert.equal(response.status, 405);
	assert.equal(response.headers.get('Allow')?.includes('MKCOL'), true);
	assert.equal(response.headers.get('DAV'), '1, 2');
});
...
test('DELETE rejects deleting the repository root', async () => {
	const bucket = createTrackingBucket([createObject('docs/readme.txt', { bodyText: 'hello' })]);
	...
	assert.deepStrictEqual(bucket.operations.delete, []);
});

test('DELETE rejects deleting the repository root before evaluating descendant locks', async () => {
	const bucket = createTrackingBucket([
		createObject('docs/file.txt', {
			bodyText: 'hello',
			customMetadata: { lock_records: JSON.stringify([LOCK_DETAIL_SAMPLE]) },
		}),
	]);
	const response = await handleDelete(new Request('http://example.com/', { method: 'DELETE' }), bucket);
...
test('DELETE rejects stale If-Unmodified-Since for file resources', async () => {
	const bucket = createTrackingBucket([
		createObject('file.txt', {
			bodyText: 'hello',
			uploaded: new Date('2024-01-02T00:00:00.000Z'),
	...
	assert.equal(response.status, 412);
	assert.equal(bucket.operations.delete.length, 0);
});
...
test('DELETE rejects unsupported complex If headers for lock evaluation', async () => {
	const bucket = createTrackingBucket([
		createObject('file.txt', {
			bodyText: 'hello',
			customMetadata: { lock_records: JSON.stringify([LOCK_DETAIL_SAMPLE]) },
	...
	const request = new Request('http://example.com/file.txt', {
		method: 'DELETE',
		headers: {
			If: `(Not <urn:uuid:${LOCK_DETAIL_SAMPLE.token}>)`,
		},
	});
...
test('DELETE accepts tagged If headers that carry the active lock token', async () => {
	const bucket = createTrackingBucket([
		createObject('file.txt', {
			bodyText: 'hello',
			customMetadata: { lock_records: JSON.stringify([LOCK_DETAIL_SAMPLE]) },
	...
	const response = await handleDelete(
		new Request('http://example.com/file.txt', {
			method: 'DELETE',
			headers: {
				If: `</file.txt> (<urn:uuid:${LOCK_DETAIL_SAMPLE.token}>)`,
			},
		}),
		bucket,
	);
	...
	assert.equal(response.status, 204);
	assert.deepStrictEqual(bucket.operations.delete, ['file.txt']);
});

test('DELETE accepts tagged If headers for an ancestor collection lock', async () => {
	...
	const response = await handleDelete(
		new Request('http://example.com/docs/file.txt', {
			method: 'DELETE',
			headers: {
				If: `</docs/> (<urn:uuid:${LOCK_DETAIL_SAMPLE.token}>)`,
			},
		}),
		bucket,
	);
	...
	assert.equal(response.status, 204);
	assert.deepStrictEqual(bucket.operations.delete, ['docs/file.txt']);
});

test('DELETE rejects tagged If headers for a different resource path', async () => {
	const sidecarPayload = serializeDirectorySidecar({
		kind: 'directory',
		locks: [LOCK_DETAIL_SAMPLE],
	});
	...
	const response = await handleDelete(
		new Request('http://example.com/docs/file.txt', {
			method: 'DELETE',
			headers: {
				If: `</other/> (<urn:uuid:${LOCK_DETAIL_SAMPLE.token}>)`,
			},
		}),
		bucket,
	);
...
test('DELETE accepts If headers that combine a lock token and a matching ETag', async () => {
	const bucket = createTrackingBucket([
		createObject('file.txt', {
			bodyText: 'hello',
			etag: 'etag-1',
	...
	const response = await handleDelete(
		new Request('http://example.com/file.txt', {
			method: 'DELETE',
			headers: {
				If: `(<urn:uuid:${LOCK_DETAIL_SAMPLE.token}> ["etag-1"])`,
			},
		}),
		bucket,
	);
...
test('file COPY rejects stale If-Unmodified-Since when overwriting an existing destination file', async () => {
	const bucket = createTrackingBucket([
		createObject('source.txt', { bodyText: 'new', uploaded: new Date('2024-01-01T00:00:00.000Z') }),
		createObject('dest.txt', { bodyText: 'old', uploaded: new Date('2024-01-02T00:00:00.000Z') }),
	]);
	const request = new Request('http://example.com/source.txt', {
		method: 'COPY',
		headers: {
	...
	assert.equal(response.status, 412);
	assert.equal(bucket.objects.get('dest.txt')?.bodyText, 'old');
});
...
test('file MOVE rejects stale If-Unmodified-Since for the source file', async () => {
	const bucket = createTrackingBucket([
		createObject('source.txt', { bodyText: 'new', uploaded: new Date('2024-01-02T00:00:00.000Z') }),
	]);
	const request = new Request('http://example.com/source.txt', {
		method: 'MOVE',
		headers: {
	...
	assert.equal(response.status, 412);
	assert.equal(bucket.objects.has('source.txt'), true);
	assert.equal(bucket.objects.has('dest.txt'), false);
});
